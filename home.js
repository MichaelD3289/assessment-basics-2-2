...
function canWeDeliver(zipCode) {
    (deliveryAreaZipCodes.includes(zipCode)) ? console.log('You\'re in our delivery zone!') : console.log('Sorry, we can\' deliver to that address')
}
...
function canWeDeliverTwo(zipCode) {
    let weCanDeliver = false
    for(let i = 0; i < deliveryAreaZipCodes.length; i++) {
        if(deliveryAreaZipCodes[i] === zipCode) {
            weCanDeliver = true
        }
    }
    (weCanDeliver === true) ? console.log('You\'re in our delivery zone!') : console.log('Sorry, we can\' deliver to that address')
}
...
// const {title} = deals[0]
// const newTitle = title.replace('15', '10')
...
// const {desc} = deals[1];
// const newDesc = desc.replace('March', 'April').trim();
